...
#directory where 15 trial videos are (each location)
directory = "carpetSlow" 

# Setup storage directories 
video_Dir_name = directory + "_Output"
IsDirExist = os.path.exists(video_Dir_name)
if not IsDirExist:
        os.mkdir(video_Dir_name)

csv_Dir_name = directory+"_csvFiles"
...
for file in sorted(os.listdir(directory)): 
    ...
    cap = cv2.VideoCapture(filename) #object used to get video frames, either live from webcam or from video file
    ...
    out = cv2.VideoWriter(video_Dir_name + "/" + directory + str(fileNum) + ".avi",cv2.VideoWriter_fourcc('M','J','P','G'), fps, (frame_width,frame_height) )
    ...
    while(cap.isOpened):
        ...
        try:
            ...
            print("Video number ", str(fileNum), "/15  ", ":: frame progress: ", frameCnt, "/", vid_length)
    ...
    np.savetxt(csv_Dir_name + "/" + directory + str(fileNum) + ".csv", distanceArray, delimiter=', ', fmt='%1.3f')
    cap.release()
